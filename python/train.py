...
def main():
    ...
    for e in range(1, 1+CFG.epoch):
        ...
        if valid_score > store_score:
            store_score = valid_score
            torch.save(model.state_dict(), 'model.pth')
    convert_to_onnx('model.pth')
...
def convert_to_onnx(pth):
    ...
    # ONNXに入力するテンソルを作成
    # x := (N, C, H, W)
    x = torch.randn(1, 3, CFG.image_size, CFG.image_size, device=CFG.device)
    # IOの名称を決める.
    # ONNXでI/Fとして使用する際に重要になるため、間違わないような名前を付けておくことを推奨.
    x_names = [ 'input1' ]
    y_names = [ 'output' ]
    # もし複数の入力が必要な場合は下記のようにする.
    # x = (
    #      torch.randn(1, 3, CFG.image_size, CFG.image_size, device=CFG.device), # 1st Input
    #      torch.randn(1, 3, CFG.image_size, CFG.image_size, device=CFG.device), # 2nd Input
    # )
    # x_names = [ 'input1', 'input2' ]
    # y_names = [ 'output' ]
    # ONNXモデルの生成
